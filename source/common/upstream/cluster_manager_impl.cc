...
ClusterManagerImpl::loadCluster(const envoy::config::cluster::v3::Cluster& cluster,
  ...
  if (!cluster_reference.info()->waitForWarmOnInit() || cluster_reference.hasSdsZeroTimeoutDependency()) {
    cluster_entry_it->second->markSdsZeroTimeout();
  }
...
void ClusterManagerImpl::updateClusterCounts() {
  ...
  if (all_clusters_initialized && !shutdown_ && xds_manager_.adsMux()) {
    ...
    for (const auto& [name, cluster_data] : warming_clusters_) {
      if (!cds_pauses_.contains(name) && !cluster_data->hasSdsZeroTimeout()) {
